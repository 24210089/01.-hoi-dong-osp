...
const assignSisterToCommunity = async (req, res) => {
  try {
    ...
    if (!sisterId || !communityId || !role || !startDate) {
      return res.status(400).json({ message: 'sister_id, community_id, role, start_date are required' });
    }
    ...
    const currentAssignment = await CommunityAssignmentModel.getCurrentAssignment(sisterId);
    ...
    return res.status(500).json({ message: 'Failed to assign sister to community' });
  }
};
...
const updateAssignment = async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const existing = await CommunityAssignmentModel.executeQuery('SELECT * FROM community_assignments WHERE id = ?', [id]);
    ...
    if (payload.community_id || payload.sister_id) {
      ...
      const validation = await ensureEntitiesExist(targetSisterId, targetCommunityId);
      if (validation.error) {
        return res.status(404).json({ message: validation.error });
      }
    }
...
const endAssignment = async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const existingRows = await CommunityAssignmentModel.executeQuery('SELECT * FROM community_assignments WHERE id = ?', [id]);
    ...
    const updated = await CommunityAssignmentModel.update(id, { end_date: finalDate });
    await logAudit(req, 'UPDATE', id, assignment, updated);
...
const getAssignmentHistory = async (req, res) => {
  try {
    ...
    return res.status(200).json({ sister: { id: sister.id, code: sister.code }, history });
  } catch (error) {
...
const getCurrentAssignment = async (req, res) => {
  try {
    ...
    const { sisterId } = req.params;
    const assignment = await CommunityAssignmentModel.getCurrentAssignment(sisterId);
    if (!assignment) {
    ...
    return res.status(500).json({ message: 'Failed to fetch current assignment' });
  }
};
...
const uploadDecisionFile = async (req, res) => {
  try {
    ...
    if (assignment.decision_file_url) {
      const oldPath = path.join(UPLOADS_ROOT, assignment.decision_file_url.replace('/uploads/', ''));
    ...
    const updated = await CommunityAssignmentModel.update(id, { decision_file_url: fileUrl });
    await logAudit(req, 'UPDATE', id, assignment, updated);
