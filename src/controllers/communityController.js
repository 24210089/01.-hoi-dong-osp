...
const getCommunityById = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Failed to fetch community detail' });
  }
};
...
const deleteCommunity = async (req, res) => {
  try {
    ...
    if (memberCount > 0) {
      return res.status(400).json({ message: 'Cannot delete community while members are assigned' });
    }
...
const getCommunityMembers = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Failed to fetch community members' });
  }
};
